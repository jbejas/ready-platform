...
export default function ValueSetupScreen({ navigation, route }: Props) {
  // Mount Localization
  ...
  return (
    <SafeAreaComponent testID="ValueSetup" relative>
      <View className="h-full w-full left-0 flex flex-col items-center justify-start">
        <Text
          allowFontScaling={false}
          className="text-[30px] sm-1:text-[36px] text-dark text-left mt-2 mb-6 font-jokkerl w-full -tracking-widest"
        >
          {t("values-screen.title")}
        </Text>
        <HorizontalReflectionQuestionsScroll
          data={responses}
          existingValue={user?.value}
